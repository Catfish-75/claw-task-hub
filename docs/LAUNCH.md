...
Use this page when announcing Claw Task Hub to agent, local-first, open-source, and MCP communities. Keep every post scoped to the real v0.1.0 product: a local-first MVP for agentic task coordination, not a hosted Linear replacement.
...
- Linear migration guide: https://github.com/Catfish-75/claw-task-hub/blob/main/docs/LINEAR_MIGRATION.md
...
The first public MVP is meant for local agent workflows with tools such as Codex, Claude Code, OpenClaw, Hermes, and compatible MCP runners. It is not a hosted multi-user SaaS product, and the API binds to localhost by default.
...
Claw Task Hub is a local-first experiment in that direction. It keeps the familiar Linear-like shape for humans, but the durable contract is optimized for agents: CLI and MCP-compatible tools, SQLite persistence, local issue identifiers, agent sessions, issue claims, comments, invariant repair, and smoke tests that prove an agent can complete a full task lifecycle without touching the UI.

The v0.1.0 MVP is intentionally narrow. It runs locally, stores data in SQLite, binds the API to localhost by default, and includes an optional one-time migration path for users who already have Linear history. The goal is to make agent task coordination boring, inspectable, and portable before expanding into broader workflows.
...
I built Claw Task Hub, a local-first task tracker for AI agents and agentic harnesses.

The idea is simple: Linear is great for human teams, but agents need a more deterministic contract. Claw Task Hub keeps a familiar project/issue UI, while exposing agent-friendly workflows for projects, issues, comments, sessions, issue claims, claim release, and acceptance trails over a local SQLite database.
...
- optional legacy Linear migration tool, outside the normal runtime

Repo: https://github.com/Catfish-75/claw-task-hub
Release: https://github.com/Catfish-75/claw-task-hub/releases/tag/v0.1.0

I would especially like feedback from people running Codex, Claude Code, OpenClaw, Hermes, or other local agent harnesses. What task state do your agents need that human-first trackers do not model well?
```
...
- optional one-time Linear migration tool

This is not a hosted SaaS product. It is meant to run locally next to tools like Codex, Claude Code, OpenClaw, Hermes, or other compatible runners.
...
```text
Claw Task Hub: a local-first task tracker for AI agents
```
...
- optional one-time Linear history migration

It is not a hosted multi-user SaaS product. It is a local operator tool for people running agents such as Codex, Claude Code, OpenClaw, Hermes, or other compatible harnesses.
...
```text
I released the first public MVP of Claw Task Hub, a local-first task hub built primarily for AI agents and agentic harnesses.
...
The v0.1.0 release is intentionally local-first. It binds to localhost by default and is meant to run next to tools such as Codex, Claude Code, OpenClaw, Hermes, or other compatible runners.
...
```text
Local-first task tracking for AI agents
```
...
The v0.1.0 MVP is intentionally local-first. It runs on localhost, stores data in SQLite, exposes CLI and MCP-compatible tools, and keeps the normal runtime independent from Linear. There is an optional migration tool for users with existing Linear history, but new work stays local.

I am looking for feedback from people running Codex, Claude Code, OpenClaw, Hermes, and other agentic harnesses.
```
